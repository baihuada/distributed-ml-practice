...
如果你的本地目录名称仍为中文，例如 `DML-Bench`、`PS-VS-AllReduce`、`IBing论文复现`，也可以正常使用；但为了 GitHub 展示和后续维护，建议统一为英文目录名。
...
```text
w_{t+1} = w_t - η g_k(w_{t-τ})
```

其中 `τ` 表示 staleness。异步更新可以缓解 straggler，但可能引入过期梯度导致训练不稳定。
